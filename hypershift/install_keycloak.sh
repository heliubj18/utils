...
function setup_keycloak () {
   ...
   # KEYCLOAK_ADMIN_TEST_USER="admin-$(< /dev/urandom tr -dc 'a-z0-9' | fold -w 6 | head -n 1 || true)"
    KEYCLOAK_ADMIN_TEST_USER="admin-$(LC_CTYPE=C tr -dc 'a-z0-9' < /dev/urandom | head -c 6)"
   # KEYCLOAK_ADMIN_TEST_PASSWORD="$(< /dev/urandom tr -dc 'a-z0-9' | fold -w 12 | head -n 1 || true)"
    KEYCLOAK_ADMIN_TEST_PASSWORD="$(LC_CTYPE=C tr -dc 'a-z0-9' < /dev/urandom | head -c 6)"
    oc process -n keycloak -f https://raw.githubusercontent.com/keycloak/keycloak-quickstarts/latest/openshift/keycloak.yaml \
        ...
        | gsed -e 's/KEYCLOAK_ADMIN_PASSWORD/KC_BOOTSTRAP_ADMIN_PASSWORD/g' -e 's/KEYCLOAK_ADMIN\b/KC_BOOTSTRAP_ADMIN_USERNAME/g' \
...
"
    ...
    oc rsh -n keycloak dc/keycloak cat /tmp/postStart.log  2>&1 | tee /tmp/postStart.log
...
EOF
    ...
    CONSOLE_CLIENT_SECRET_VALUE=$(LC_CTYPE=C tr -dc 'a-z0-9' < /dev/urandom | head -c 32)
...
function prepare_keycloak_user_data () {
    ...
    do
        ...
        password=$(LC_CTYPE=C tr -dc 'a-z0-9' < /dev/urandom | head -c 6)
